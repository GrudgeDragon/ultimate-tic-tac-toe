...
class UT3Game:
    ...
    def make_move(self, agent: UT3Agent):
        # make move
        self.move += 1
        move = agent.make_move(self.global_board, self.next_local_board)
        # validate
        if not self.validate_move(self.global_board, self.next_local_board):
            # TODO: Handle bad moves
            print("Handle b")
            return False
        # apply to board
        self.global_board[move[0], move[1]] = agent.player_num
        # check for win condition
        winner = get_global_winner(self.global_board)

        #TODO: Enforce next move restrictions
    ...
    def validate_move(self, move, next_local_board):
        return True